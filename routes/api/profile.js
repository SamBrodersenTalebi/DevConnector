...
router.get('/me', auth, async (req, res) => {
  try {
    //populate method will populate the user field with name and avatar
    const profile = await Profile.findOne({
      user: req.user.id
    }).populate('user', ['name', 'avatar']);

    //In case there is no profile
    if (!profile) {
      return res.status(400).json({ msg: 'There is no profile for this user' });
    }

    //send along profile if there is one
    res.json(profile);
  } catch (err) {
...
router.get('/user/:user_id', async (req, res) => {
  try {
    ...
    //check to see if there is a profile for the user
    if (!profile) {
      return res.status(400).json({ msg: 'There is no profile for this user' });
    }
    res.json(profile);
...
// @desc    Get Github repos for profile
// @access  Public
